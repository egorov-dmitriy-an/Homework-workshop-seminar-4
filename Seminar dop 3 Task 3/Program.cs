// Задача 3. На ввод подаётся номер четверти.
// Создаются n случайных точек в этой четверти. 
...
namespace ConsoleApp
{
    class Program
    {
        ...
        {
            Console.WriteLine("");
            Console.WriteLine("================ Семинар 3 Задача 3 Доп ================");
            int[] PointsX(int quarter_number_X, int n_point_1) // Метод для определения координаты Х точки
            {
                int[] cur_coordX = new int[n_point_1 + 1];
                int mod_x = 0;
            ...
            int[] PointsY(int quarter_number_Y, int n_point_2) // Метод для определения координаты Y точки
            {
                int[] cur_coordY = new int[n_point_2 + 1];
                int mod_y = 0;
            ...
            Console.Write("Введите количество точек: "); // Введение количества точек
            int n_point_3 = Convert.ToInt32(Console.ReadLine());

            int[] coordX = new int[n_point_3 + 1]; 
            int[] coordY = new int[n_point_3 + 1];
            Console.Write("Введите номер четверти: "); // Введение номера четверти
            int quarter = Convert.ToInt32(Console.ReadLine());
            if (quarter < 1 || quarter > 4) Console.WriteLine("Введен неправильный номер четверти");
            else
            {
                coordX = PointsX(quarter, n_point_3);
                coordY = PointsY(quarter, n_point_3);
            }
            for (int i = 0; i <= n_point_3; i++)
            {
